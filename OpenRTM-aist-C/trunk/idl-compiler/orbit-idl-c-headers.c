...
ch_output_type_struct(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci, int defcpp)
{
  ...
  if(rinfo->cpp_defs){
    ...
    fprintf(ci->fh, "  ~%s_type() { RtORB_free_by_typecode(TC_%s, this, 0, 1);}\n", id, id);
...
ch_prep_sequence(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci)
{
  ...
  if(separate_defs)
  {
    ...
    if(rinfo->cpp_defs){
      ...
      fprintf(ci->fh, "  ~%s_type () {\n", typespec_str);
      fprintf(ci->fh, "    if (_buffer) { RtORB_free_by_typecode(_type_code(),this,0,1); }\n");
	...
	break;
      ...
      fprintf(ci->fh, "      RtORB_free_by_typecode(_type_code(), this, 0, 1);\n");
      fprintf(ci->fh, "    }\n ");
