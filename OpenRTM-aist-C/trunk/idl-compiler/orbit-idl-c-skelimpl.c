...
static void orbit_cbe_write_skelimpl_h(FILE *outfile, IDL_tree tree, const char *hdrname);
...
void
orbit_idl_output_h_skelimpl(IDL_tree tree, OIDL_Run_Info *rinfo, OIDL_C_Info *ci)
{
  info = rinfo;
  orbit_cbe_write_skelimpl_h(ci->fh, tree, ci->base_name);
}
...
orbit_cbe_write_skelimpl(FILE *outfile, IDL_tree tree, const char *hdrname)
{
  ...
  fprintf(outfile, "#include \"%s-skelimpl.h\"\n", hdrname);

//  for(ski.pass = PASS_SERVANTS; ski.pass < PASS_LAST; ski.pass++) {
  for(ski.pass = PASS_EPVS; ski.pass < PASS_LAST; ski.pass++) {
    fprintf(outfile, "\n/*** %s ***/\n", passnames[ski.pass]);
...
static void
orbit_cbe_write_skelimpl_h(FILE *outfile, IDL_tree tree, const char *hdrname)
{
  CBESkelImplInfo ski = {NULL, NULL, PASS_SERVANTS};
  ...
  fprintf(outfile, "#ifndef %s_SKELIMPL_H\n", UpcaseStr(hdrname));
  fprintf(outfile, "#define %s_SKELIMPL_H\n\n", UpcaseStr(hdrname));

  fprintf(outfile, "#include \"%s.h\"\n", hdrname);

//  for(ski.pass = PASS_SERVANTS; ski.pass < PASS_LAST; ski.pass++) {
  for(ski.pass = PASS_SERVANTS; ski.pass < PASS_EPVS; ski.pass++) {
    fprintf(outfile, "\n/*** %s ***/\n", passnames[ski.pass]);
    orbit_cbe_ski_process_piece(&ski);
  }
...
cbe_ski_do_op_dcl(CBESkelImplInfo *ski)
{
  ...
  case PASS_IMPLSTUBS:
	...
	fprintf(ski->of, "  memset(ev, 0, sizeof(CORBA_Environment));\n");
	  ...
	  fprintf(ski->of, "    /* ------   insert method code here  (1) ------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
          fprintf(ski->of, "#endif /* __cplusplus */ ");
 	}else{
	  fprintf(ski->of, "    /* ------   insert method code here (2)  ------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
	}
	fprintf(ski->of, "\n  return retval;\n");
      }
	  ...
	  fprintf(ski->of, "    impl->%s(", func);
  	  ...
  	  fprintf(ski->of, ");\n");
	  fprintf(ski->of, "#else /* C */\n");
	  fprintf(ski->of, "    /* ------   insert method code here  (3) ------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
	  fprintf(ski->of, "#endif /*__cplusplus */\n");
        }else{
	  fprintf(ski->of, "    /* ------   insert method code here  (4) ------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
        }
...
cbe_ski_do_inherited_op_dcl(CBESkelImplInfo *ski, IDL_tree current_interface)
{
  ...
  case PASS_IMPLSTUBS:
	...
	fprintf(ski->of, " retval;\n");
	  ...
	  fprintf(ski->of, "  ev->_cpp_flag = 1;\n");
	  fprintf(ski->of, "#else /*  C  */\n");
	  fprintf(ski->of, "    /* ------   insert method code here   (5)------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
          fprintf(ski->of, "#endif /* __cplusplus */ ");
        }else{
	  fprintf(ski->of, "    /* ------   insert method code here  (6) ------ */\n");
	  fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
        }
	fprintf(ski->of, "\n  return retval;\n");
      } else {	
	fprintf(ski->of, "    /* ------   insert method code here   (7)------ */\n");
	fprintf(ski->of, "    /* ------ ---------- end ------------ ------ */\n");
      }
      fprintf(ski->of, "}\n\n");
    } else { /* PASS_PROTOS */
...
//    fprintf(ski->of, "static void impl_%s__destroy(\n  impl_POA_%s *servant,\n  CORBA_Environment *ev);\n\n", id, id);
    fprintf(ski->of, "void impl_%s__destroy(\n  impl_POA_%s *servant,\n  CORBA_Environment *ev);\n\n", id, id);
    subski.tree = IDL_INTERFACE(ski->tree).body;
    cbe_ski_do_list(&subski);
    IDL_tree_traverse_parents(ski->tree, (GFunc)&cbe_ski_do_inherited_methods,
    				ski);
