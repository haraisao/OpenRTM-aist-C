...
#ifdef METHOD_LOG
void
printInvokeServent(char *fname, char *class_name, char *function){
  FILE *fp;
  char str1[128];
  char str2[128];
  char ffname[32];

  sprintf(str1, "%s@%s", class_name, function);
  sprintf(ffname, "%s_%d.log", fname, (int)getpid());
		...
		octet *arg_buf, int version, int order)
{
  ...
  if(poa_obj == NULL){
    ...
    RtORB_Result__free(m_data->retval, result, env->_cpp_flag);
